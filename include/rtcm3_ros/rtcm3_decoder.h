...
public:
  ...
  public:
    ...
    Buffer(const std::vector<uint8_t> in)
...
protected:
  Buffer raw_buffer_;
  static unsigned int getUnsignedBits(
      const Buffer &data, const size_t pos, const size_t len)
  {
    unsigned int bits = 0;
    for (size_t i = pos; i < pos + len; i++)
    {
      bits = (bits << 1) + ((data[i / 8] >> (7 - i % 8)) & 1u);
    }
    return bits;
  }
  static int getSignedBits(
      const Buffer &data, const size_t pos, const size_t len)
  {
    unsigned int bits = getUnsignedBits(data, pos, len);
    if (!(bits & (1u << (len - 1))))
    {
      return (int)bits;
    }
...
public:
  void operator<<(const Buffer &input)
  {
    ...
    for (auto it = raw.begin(); it != raw.end(); ++it)
    {
      ...
      const size_t length = getUnsignedBits(Buffer(it, it + 3), 14, 10);
      ...
      if (CRC24Q(msg) != getUnsignedBits(crc, 0, 24))
        continue;
  ...
  {
    const int type = getUnsignedBits(msg, 24, 12);
